...
\textbf{Definition:} Hardware can reorder reads and writes as long as the result is correct from \textbf{the executing thread's perspective}. Weak ordering does not concern itself with guaranteeing correctness across executing threads.

\begin{itemize}
    \item We've limited the set of possibilities
    \item Acquire --- all subsequent reads/writes in the current thread happen after
    \item Release --- All preceding reads/writes in the current thread happen before
\end{itemize}
...
\textbf{Template: } Safety, liveness, efficiency to answer these questions.
