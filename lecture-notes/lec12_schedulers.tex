...
\textbf{Key Idea:} \emph{Reduce the priority of processes that always consume full CPU.} Conversely, boost priority of processes that frequently sleep.

\subsubsection*{Weighted Fair Queuing (WFQ)}

CFS is Linux's implementation of \textbf{Weighted Fair Queuing (WFQ)}:
\begin{itemize}
    \item Resources are shared \emph{proportionally} to each process's \textbf{weight}.
...
\begin{itemize}
    ...
    \end{itemize}
    \item \textbf{Soft real-time:} Critical processes receive priority over less critical ones, but occasional deadline misses are tolerable (e.g., video streaming, interactive audio).
\end{itemize}
